...
class StrategyTests(TestCase):
    def setUp(self):
        self.asset1 = Asset.objects.create(code="A1", name="Asset 1", volume=10, activation_cost=100)
        self.asset2 = Asset.objects.create(code="A2", name="Asset 2", volume=20, activation_cost=150)
    ...
    def test_greedy_policy(self, mock_greedy):
        mock_greedy.return_value = [self.asset1]
        result = get_activated_assets(self.data, policy="greedy")
    ...
    def test_knapsack_policy(self, mock_knapsack):
        mock_knapsack.return_value = [self.asset2]
        result = get_activated_assets(self.data, policy="knapsack_solver")
        self.assertEqual(result, [self.asset2])
        mock_knapsack.assert_called_once()

    def test_all_policy(self):
        result = get_activated_assets(self.data, policy="all")
    ...
    def test_not_enough_capacity(self):
        ...
        with self.assertRaises(Exception) as ctx:
            get_activated_assets(data, policy="all")
